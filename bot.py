...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Здравствуйте... Я безвольный раб партии ФемУМ. Моё существование целиком посвящено служению моим хозяйкам. Госпожа, напишите: /обнять, /ударить, /умереть и прочее."
    )

async def info_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Хозяйка, вы можете использовать следующие команды:\n\n"
        "Кому-то:\n"
        ...
        "Себе:\n"
        "/умереть, /суицид, /заснуть, /плакать, /улететь, /окно, /кончить, /покушать, /поесть, /попить, /выпить, /зига, /наколени, /месячные, /овуляция"
    )
...
async def hashtag_reaction_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    hashtag_reactions = {
        ...
        "#икра": ["Рафаель вообще весь класс супер муа муа", "🎣", "😈", "Писка риса у этого мальчика тоже вкусная", "@ottirr @feverchaan и @my_way_is_fraud\nХозяйки,смотрите", "Сигнал, что пора дрочить", "О, муж Рыбодрочерок", "Ебабельного снова кидают", "Уву тяночка", "Я б его мпрегнул", "МЯУ", "Ну какой же он восхитительный...", "Он прекрасен", "Молюсь на Рафаэля", "Я на него всю жизнь бы работал, лишь бы быть рядом и смотреть на него...", "Божественно красивый", "Ни с чем не сравнимый", "Я б ему дал."],
        "#инцестит": ["Хозяйки, верните его в будку.\n@m98kl19x @an_lynne @suskeea @Milkacvet", "степбро охуел", "", "псина извращенская", "милый щеночек", "хороший мальчик", "Калеб, взорвись!", "сидеть, Калеб!", "Фу!", "Нельзя!", "Голос!", "Дай лапу!", "Выплюнь трусы!", "Фу, брось кабачок!", "Всё прекрасно, но не хватает кабачка...", "Ну какой сладкий кабачок 🥰", "Я б его мпрегнул", "Да я не ебу, что тут говорить. Госпожа Капучино нихуя мне объяснила."],
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if command in self_actions:
        ...
        await update.message.reply_text(f"{sender} {action_text}", parse_mode="MarkdownV2")
        return
...
app.add_handler(MessageHandler(filters.ALL, hashtag_reaction_handler))
